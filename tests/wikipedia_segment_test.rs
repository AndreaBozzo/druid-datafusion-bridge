...
fn test_read_column_descriptor() {
    ...
    let json_len = u32::from_be_bytes([time_data[0], time_data[1], time_data[2], time_data[3]]) as usize;
    ...
    let descriptor: ColumnDescriptor = serde_json::from_str(json_str)
        .expect("Failed to parse column descriptor");
    
    assert_eq!(descriptor.value_type, druid_datafusion_bridge::segment::column_descriptor::ValueType::Long);
...
fn test_read_string_column_descriptor() {
    ...
    assert_eq!(descriptor.value_type, druid_datafusion_bridge::segment::column_descriptor::ValueType::String);
    assert!(!descriptor.has_multiple_values);
    assert_eq!(descriptor.parts.len(), 1);
...
fn test_read_metadata_drd() {
    ...
    let metadata_data = reader.map_file("metadata.drd").expect("Failed to read metadata.drd");
    ...
    let metadata: serde_json::Value = serde_json::from_str(metadata_str)
        .expect("Failed to parse metadata.drd JSON");
...
fn test_read_index_drd_structure() {
    ...
    let index_data = reader.map_file("index.drd").expect("Failed to read index.drd");
    
    println!("index.drd size: {} bytes", index_data.len());
    println!("First 20 bytes: {:02x?}", &index_data[..20.min(index_data.len())]);
    ...
    let total_bytes = u32::from_be_bytes([index_data[2], index_data[3], index_data[4], index_data[5]]) as usize;
    let num_elements = u32::from_be_bytes([index_data[6], index_data[7], index_data[8], index_data[9]]) as usize;
    
    println!("GenericIndexed: version={}, flags={:#x}, total_bytes={}, num_elements={}", 
    ...
    for &end_offset in &offsets {
        ...
        if element_data.len() > 4 {
            ...
            let str_end = str_part.iter().position(|&b| b == 0).unwrap_or(str_part.len());
    ...
    let expected = vec![
        "channel", "cityName", "comment", "countryIsoCode", "countryName",
        "isAnonymous", "isMinor", "isNew", "isRobot", "isUnpatrolled",
        "metroCode", "namespace", "page", "regionIsoCode", "regionName",
        "user", "added", "deleted", "delta"
    ];
...
fn test_list_all_files() {
    ...
    let expected_columns = vec![
        "__time", "added", "channel", "cityName", "comment", "countryIsoCode",
        "countryName", "deleted", "delta", "isAnonymous", "isMinor", "isNew",
        "isRobot", "isUnpatrolled", "metroCode", "namespace", "page",
